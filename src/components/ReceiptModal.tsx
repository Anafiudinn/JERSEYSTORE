  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 print:p-0">
      <motion.div 
        ...
        className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden flex flex-col print:shadow-none print:rounded-none"
      >
        <div className="p-6 border-b border-slate-100 flex items-center justify-between print:hidden">
          <h3 className="font-black text-slate-900 tracking-tight">E-Receipt</h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 print:p-4" ref={receiptRef}>
          ...
          <div className="border-t border-b border-dashed border-slate-200 py-4 mb-6 space-y-1">
            <div className="flex justify-between text-[11px] font-bold">
              ...
              <span className="text-slate-900">{new Date(transaction.timestamp).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-[11px] font-bold">
              <span className="text-slate-400 uppercase tracking-wider">Customer</span>
              <span className="text-slate-900">{transaction.customerName || 'General Customer'}</span>
            </div>
            <div className="flex justify-between text-[11px] font-bold">
              <span className="text-slate-400 uppercase tracking-wider">TX ID</span>
          ...
          <div className="bg-slate-50 rounded-2xl p-6 space-y-3 print:bg-white print:border print:border-slate-200">
            ...
            <div className="flex justify-between text-[11px] font-bold">
              ...
              <span className="text-slate-900 text-emerald-600">{formatCurrency(transaction.change)}</span>
            </div>
            <div className="flex justify-between text-[11px] font-bold">
              <span className="text-slate-400 tracking-wider">Metode Bayar</span>
              <span className="text-slate-900 uppercase">{transaction.paymentMethod}</span>
            </div>
          </div>
        ...
        <div className="p-8 border-t border-slate-100 flex gap-4 print:hidden">
          <button 
            onClick={onClose}
            className="flex-1 px-6 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-sm transition-all"
          >
            Close
          </button>
          <button 
